        ...
        timer.scheduleAtFixedRate(new TimerTask() {
            public void run() {
                ...
                if (WPM.duration == 0) {
                    timer.cancel();
                    thread.interrupt();
                    cleanVariablesAndShowResultScreen(false, Login.IDPlayer);
                }else
...
//                    System.out.print("Time: "+  WPM.duration + "s | Net WPS: " + netWPM + " | Accuracy: " + String.format("%.2f", accuracy) + "%\n");
    ...
    void cleanVariablesAndShowResultScreen(boolean isEndOfWords,int IDPlayer){
        ...
        JDBC.InsertValues(CalculationThread.x_view.getLvl(),netWPM,cCC,wCC,aCC, (int)accuracy,IDPlayer);
